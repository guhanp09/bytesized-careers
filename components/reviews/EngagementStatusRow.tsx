    ...
    <section className="mb-5 rounded-xl border border-white/[0.09] bg-white/[0.025] px-3.5 py-3" data-testid="engagement-status-row">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
          ...
          {actions.has("cancel_before_start") ? <button type="button" aria-label="Cancel engagement" title="Cancel engagement" disabled={busy} onClick={() => void run(() => cancelEngagementBeforeStart(accessToken, engagement.id))} className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-white/10 text-subtle transition-colors hover:border-rose-200/20 hover:text-rose-200/70 disabled:opacity-40"><Icon name="close" className="h-3.5 w-3.5" /></button> : null}
        </div>
      </div>
      ...
      {panel === "outcome" ? (
        <div className="mt-3 border-t border-white/[0.06] pt-3">
          ...
          <textarea value={note} onChange={(event) => setNote(event.target.value)} rows={2} maxLength={500} placeholder="Optional context" className="mt-2 w-full resize-none rounded-lg border border-white/[0.09] bg-black/20 px-3 py-2 text-[11.5px] text-white/75 placeholder:text-subtle focus:border-white/20 focus:outline-none" />
      ...
      {panel === "issue" ? (
        <div className="mt-3 border-t border-white/[0.06] pt-3">
          ...
          <textarea id="engagement-issue-note" value={note} onChange={(event) => setNote(event.target.value)} rows={2} minLength={10} maxLength={1000} placeholder="This note stays private to the conversation." className="mt-2 w-full resize-none rounded-lg border border-white/[0.09] bg-black/20 px-3 py-2 text-[11.5px] text-white/75 placeholder:text-subtle focus:border-white/20 focus:outline-none" />
