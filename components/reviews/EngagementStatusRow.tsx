...
export default function EngagementStatusRow({
  ...
  return (
    <section className="mb-5 rounded-xl border border-line bg-wash px-3.5 py-3" data-testid="engagement-status-row">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-line bg-raised text-white/55">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[10.5px] leading-relaxed text-subtle">{detail}</p>
          </div>
        </div>
        <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
          {actions.has("decline_start") ? <button type="button" disabled={busy} onClick={() => void run(() => respondToEngagementStart(accessToken, engagement.id, "not_started"))} className="h-8 cursor-pointer rounded-lg border border-line px-3 text-[11px] font-semibold text-white/55 transition-colors hover:bg-raised hover:text-white disabled:opacity-40">Not started</button> : null}
          {actions.has("flag_completion_issue") ? <button type="button" disabled={busy} onClick={() => setPanel("issue")} className="h-8 cursor-pointer rounded-lg border border-line px-3 text-[11px] font-semibold text-white/55 transition-colors hover:bg-raised hover:text-white disabled:opacity-40">Needs attention</button> : null}
          ...
          {actions.has("cancel_before_start") ? <button type="button" aria-label="Cancel engagement" title="Cancel engagement" disabled={busy} onClick={() => void run(() => cancelEngagementBeforeStart(accessToken, engagement.id))} className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-line text-subtle transition-colors hover:border-rose-200/20 hover:text-rose-200/70 disabled:opacity-40"><Icon name="close" className="h-3.5 w-3.5" /></button> : null}
        </div>
      </div>
      ...
      {panel === "outcome" ? (
        <div className="mt-3 border-t border-line pt-3">
          ...
          <textarea value={note} onChange={(event) => setNote(event.target.value)} rows={2} maxLength={500} placeholder="Optional context" className="mt-2 w-full resize-none rounded-lg border border-line bg-black/20 px-3 py-2 text-[11.5px] text-white/75 placeholder:text-subtle focus:border-line-strong focus:outline-none" />
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            <button type="button" disabled={busy} onClick={() => void run(() => requestEngagementCompletion(accessToken, engagement.id, "ended_after_start", note))} className="h-8 cursor-pointer rounded-lg border border-line px-3 text-[11px] font-semibold text-white/60 hover:bg-raised disabled:opacity-50">Ended after start</button>
            <button type="button" onClick={() => setPanel(null)} className="h-8 cursor-pointer px-2 text-[11px] text-subtle hover:text-white/65">Cancel</button>
          </div>
        </div>
      ) : null}
      ...
      {panel === "issue" ? (
        <div className="mt-3 border-t border-line pt-3">
          ...
          <textarea id="engagement-issue-note" value={note} onChange={(event) => setNote(event.target.value)} rows={2} minLength={10} maxLength={1000} placeholder="This note stays private to the conversation." className="mt-2 w-full resize-none rounded-lg border border-line bg-black/20 px-3 py-2 text-[11.5px] text-white/75 placeholder:text-subtle focus:border-line-strong focus:outline-none" />
