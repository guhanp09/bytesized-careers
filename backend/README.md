...
- `JOB_IMPORT_SYSTEM_ATTEMPT_LIMIT` (shared rolling 30-day extraction admissions; default 1,000, range 1–100,000). This is not a calendar-month, token or currency budget: each admission includes at most `1 + OPENAI_MAX_RETRIES` provider attempts, each with the existing bounded input/output and deadline. The fixed Redis key is shared across accounts/processes and survives application restarts and limit changes. Duplicate/finished reads and local-capacity refusals spend no global unit. Global refusal rolls back the user's daily quota and tentative lease/attempt. A successful or uncertain Redis reservation is never refunded after later cancellation/DB/provider failure, conservatively preventing double spending across stores. This short bounded Redis operation precedes the processing commit; no database transaction spans provider work. Production Redis persistence/no-eviction/recovery and provider-account billing limits remain operator gates: lost Redis history would reset the allowance. Do not flush the bucket to unblock traffic. The normal test environment bypasses shared request limits; dedicated budget tests explicitly enable enforcement. Completed drafts stay readable when the allowance is exhausted or Redis is unavailable. Currency accounting still requires operator-supplied prices and billing reconciliation.
