...
test("every indexed profile route resolves through the real resolver", async () => {
  // Checking the handle exists in the manifest proves the index is
  // self-consistent. This proves the route actually works — that following the
  // link a QA brief hands someone lands on a profile rather than "not found".
  for (const name of SCENARIO_NAMES) {
    for (const entry of manifest(name).index ?? []) {
      const [pathname] = entry.route.split("?");
      if (!pathname.startsWith("/u/")) continue;
      const slug = pathname.slice(3);
      const profile = await getCanonicalPublicProfile(slug, { anchorMode: "fixed" });
      assert.ok(profile, `${name}: indexed route ${entry.route} does not resolve`);
      assert.ok(
        profile.bio || profile.hiring_info,
        `${name}: ${entry.route} resolves to an empty page`
      );
    }
  }
});

test("an indexed profile condition is backed by data that satisfies it", async () => {
  // The index makes claims — "six pieces of evidence", "every item described".
  // A condition the data does not meet sends a reviewer to check something
  // that is not there, which is worse than leaving it unindexed.
  const entries = (manifest("default").index ?? []).filter((entry) =>
    entry.route.startsWith("/u/")
  );
  assert.ok(entries.length > 0, "no profile route indexed");

  for (const entry of entries) {
    const profile = await getCanonicalPublicProfile(entry.route.slice(3), {
      anchorMode: "fixed",
    });
    if (/pieces of evidence|deep enough to scroll/i.test(entry.expected_condition)) {
      assert.ok(
        profile.portfolio_now.length >= 5,
        `${entry.expected_condition}: only ${profile.portfolio_now.length} items`
      );
      for (const item of profile.portfolio_now) {
        assert.ok(item.description, `${item.title} has no description`);
      }
    }
    if (/hiring identity/i.test(entry.expected_condition)) {
      assert.ok(profile.hiring_info, "no hiring identity");
      assert.ok(profile.hiring_info.channels_or_pages_managed, "no description to read");
      assert.ok(profile.bio, "no channel bio");
    }
  }
});
