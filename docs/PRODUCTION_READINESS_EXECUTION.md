...
| RATE-004 | HIGH | Request safety | None for body bounds; concurrency parts need RATE-001 | NOT_STARTED | Body-size, timeout, concurrency, cancellation and recovery tests | NO | YES | Yes | Yes | Body-size bounds do NOT depend on RATE-001 and are the next locally implementable Phase 3 slice. Defect located and recorded in the handoff under "RATE-004 request-body bounds": nothing bounds an inbound body; avatar/banner upload arrives as a base64 data URL in JSON with `data_url: str` carrying no `max_length`, and the 5 MiB check in `profile_service.py:1963` runs after the body is read, parsed and decoded — too late to protect memory. Needs an ASGI middleware bounding the body before it is read, answering 413 in the canonical error envelope |
