...
| AI-004 | CRITICAL | Budget/cost controls | AI-001 | IN_PROGRESS | Model allowlist, token/output cap, request/system budget and threshold tests | POSSIBLE | YES | Yes | Yes | Model allowlist and output cap done ("feat(import): refuse a model nobody chose, and say so before a recruiter waits"; the cap predates this work in the adapter). A spend budget with thresholds still needs somewhere to record spend and is NOT done |
