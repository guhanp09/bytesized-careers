...
| ID | Severity | Subsystem | Dependencies | Status | Tests required | Migration | Infra | Blocks beta | Blocks unrestricted | Resolving commit |
|---|---|---|---|---|---|---|---|---|---|---|
| DEP-001 | CRITICAL | Frontend dependencies | BASE-003 | IN_PROGRESS | Focused framework/auth tests, full frontend suites, production audit | NO | NO | Yes | Yes | Phase 3B upgrades `next-auth` 4.24.14 → 4.24.15, clearing the critical homoglyph-normalizer advisory, the `getToken()` malformed-Bearer crash that the administrator strong-auth route is exposed to, the unbound OAuth state/nonce/PKCE cookie advisory, and the transitive `uuid` bounds-check advisory. Production findings 6 → 4. `next` 16.2.6 → 16.3.1 is DEP-001B and carries the postcss/sharp/nanoid transitives with it |
