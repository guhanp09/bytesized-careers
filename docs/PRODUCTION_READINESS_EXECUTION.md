...
| AI-006 | HIGH | Boot/readiness | AI-001 | IN_PROGRESS | Missing key, queue, model and provider readiness tests | NO | NO | Yes | Yes | Credential and model readiness done, on GET /api/v1/health/job-import ("feat(import): refuse a model nobody chose, and say so before a recruiter waits"). Queue readiness cannot exist until AI-001 does |
