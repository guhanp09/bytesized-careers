...
| AUTH-006 | CRITICAL | Session revocation | AUTH-005 | IN_PROGRESS | Logout, logout-all, password reset, suspension, credential-change tests | YES | NO | Yes | Yes | Phase 1C-4 validates authoritative `sid` access checks, refresh-proved current logout, logout-all, and revocation/refresh concurrency; password-reset and suspension all-family triggers plus active-session management remain |
