...
| WEB-008 | HIGH | HTTP boundaries | WEB-005, WEB-007 | IN_PROGRESS | CSP, Trusted Host, CORS, COOP/CORP and production header tests | NO | YES | Yes | Yes | Phase 2I adds `frame-ancestors 'none'`, COOP `same-origin-allow-popups` and CORP `same-origin` on top of the existing nosniff/Referrer-Policy/Permissions-Policy/HSTS baseline, with seven contract tests including wildcard, env-splicing and HSTS-gating guards; the full `script-src`/`connect-src`/`img-src`/`frame-src` policy is WEB-008B and needs an inline-script decision plus browser CSP-violation verification |
