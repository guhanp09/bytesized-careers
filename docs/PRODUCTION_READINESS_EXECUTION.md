...
| WEB-006 | CRITICAL | User-supplied URLs | BASE-003 | IN_PROGRESS | Portfolio, profile, media, social, application, interview malicious URL tests | POSSIBLE | NO | Yes | Yes | Phase 2G adds canonical `app.core.external_url`, routes the existing `_is_http_url` service chokepoint through it, and adds schema enforcement for the portfolio and profile URL fields that previously had no validation at all, with 36 focused cases; render-safe external navigation (WEB-006B) and the remaining application/interview URL fields remain |
