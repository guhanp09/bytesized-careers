...
| OF-007 | `lib/youtubeIdentity` custom-path channel lookup | User URL is restricted syntactically to known YouTube hosts | None | No longer fetches: the caller injects a resolver backed by backend `POST /me/organization-page`, which reads the page through the shared pinned boundary and returns only the channel id; without that resolver a custom path falls back to URL-derived identity rather than fetching from the Next runtime | Nothing outstanding for this caller; Phase 3M also moved its subsequent fixed Data API lookup behind the backend boundary | WEB-003 | MIGRATED_PHASE_2D_3 |
...
| OF-103 | Backend `YouTubeProviderClient` → exact channels/videos Data API constants | OAuth `mine=true` has no user selector; channel/video selectors are type- and length-bounded before encoding and cannot select a destination | OAuth bearer header for owned-channel refresh or backend-only `SecretStr` API key in fixed query | Redirect-free, environment-proxy-independent, fresh cookie-free request; two-second connect/eight-second operation and twelve-second whole-resolution deadlines; 512 KiB decoded-body ceiling; bounded strict JSON and field/list/cardinality normalization; privacy-safe errors. Only 401 or an explicit bounded 403 `authError`/`insufficientPermissions` becomes reauthentication; redirects/quota/network/malformed responses preserve stored authority | Restrict the production key to the backend/API, set provider quota, and run live OAuth/key/timeout/outage drills; provider alert delivery remains external | WEB-008, RATE-004 |
| OF-104 | Same-origin authenticated Next organization resolver → Redis-rate-limited backend `POST /me/youtube-identity` → `YouTubeProviderClient` | Next parses a supported YouTube URL into one bounded selector; custom paths first obtain only a channel ID from OF-007 | No provider key or provider OAuth credential exists in the Next environment/client/response; only the caller's CreatorJobs backend bearer crosses the internal proxy | Next utility contains no fetch/API URL/key logic. Backend route shares the 30/10-minute verified-user outbound allowance, returns only a bounded normalized identity, and inherits OF-103's fixed-provider boundary. Missing key, no result, invalid provider data and outage retain the existing URL-derived fallback | Move the production key from any frontend runtime into backend secret configuration before enabling enrichment; complete Google console restriction/quota and live fallback/outage validation | WEB-003, WEB-008, RATE-003, RATE-004 |
