...
| OF-003 | `link_preview_service._fetch_text_url` via authenticated `POST /portfolio/link-preview` | User supplies the entire portfolio URL | None | Duplicated hostname/DNS checks; HTTP client can resolve again, honors environment proxies, truncates rather than rejects oversized bodies, and has incomplete type/redirect controls | Migrate HTML preview to `SafeOutboundFetcher`; preserve graceful manual-entry fallback and add full integration/security tests | WEB-004 | NEXT_PHASE_2B |
| OF-004 | `link_preview_service._fetch_oembed_json` | Work URL is user input; endpoint starts as fixed YouTube/Vimeo but automatic redirects may escape | None | Initial endpoint is checked, but `follow_redirects=True`, response size/type are unbounded, and environment proxies are honored | Add a shared bounded JSON policy plus a strict fixed-host redirect contract; never send provider/user credentials | WEB-002, WEB-004 | NEXT_PHASE_2B |
