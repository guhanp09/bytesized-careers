...
```text
LAST COMPLETED PHASE: Phase 12 — locally implementable observability, incident-response and credential-rotation work is complete; hosted restore/provider/human/soak gates remain explicit, and OPS-005 still needs one quiet-host 6/6 synthetic aggregate plus live delivery proof
CURRENT PHASE: Phase 13 — final release certification
LAST COMPLETED ATOMIC SLICE: Phase 12E — all 18 credential families now have a machine-checked declaration/enforcement/consumer inventory, planned-versus-compromise procedures, locally rehearsed OAuth/TOTP keyring rewrap, and bounded old/current/retired shared-secret behavior. The implementation commit is named `ops(credentials): make secret rotation rehearsable`; resolve the exact current HEAD with `git rev-parse HEAD` because this handoff is committed with the implementation. OPS-007 is VALIDATED locally; no real credential/provider was rotated.
NEXT ATOMIC SLICE: Phase 13A — run the complete local certification matrix from the execution program, beginning with whole-repository static/unit/backend/Ruff gates and PostgreSQL migration evidence, then browser suites serially. Repair only certification failures and keep every genuinely external gate explicit.
...
KNOWN TEST FAILURES: no Phase 12E task-caused failure remains: focused backend configuration/shared-secret/keyring suite 165 passed/1 skipped; broad auth/OAuth/Google/email/config selection 600 passed/16 skipped; credential/image/incident frontend contract 16/16; TypeScript and production build green; lint 0 errors/32 existing warnings; changed Python Ruff exit 0. Expected current collections are backend 7,541 and frontend node 1,289. Phase 13 owes the complete aggregates rather than inferring them from focused runs; last complete observed aggregates remain backend 7,506/exit 0 and frontend 1,276/1,276 from Phase 12A. Phase 12C browser truth is unchanged: OPS-005 stays IMPLEMENTED until a quiet-host 6/6. Whole-backend Ruff remains a known local Phase 13 blocker with 67 findings outside Phase 12 touched files. Phase 11L browser evidence remains canonical 85/87 with each exact Firefox host timeout green isolated.
COMMANDS TO RESUME: `git status --short`; `git branch --show-current`; `git rev-parse HEAD`; from backend `.venv/bin/python -m alembic heads` and `APP_ENV=test .venv/bin/python -m pytest`; from root run the Phase 13 commands in the execution program serially. Do not run two pytest processes against the shared test database or any build concurrently with Playwright against `.next`. Run `npm run test:e2e:synthetics` on a quiet host before promoting OPS-005.
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; this Phase 12E checkpoint; docs/CREDENTIAL_ROTATION.md; docs/INCIDENT_RESPONSE.md; backend/scripts/rotate_strong_auth_secrets.py; backend/scripts/rotate_oauth_credentials.py; next.config.ts; Phase 13 acceptance commands in the user program
