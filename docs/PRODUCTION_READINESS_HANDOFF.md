...
LAST COMPLETED ATOMIC SLICE: Phase 2B — portfolio HTML and fixed-provider oEmbed previews on the shared boundary, with bounded page metadata and preserved manual-entry fallback
NEXT ATOMIC SLICE: Phase 2C — move `ProfileService._fetch_public_hiring_identity_text` (OF-005) onto the shared boundary while preserving the platform host/path allowlist and verification semantics; keep the unauthenticated Next organization resolver (OF-006/OF-007) for the slice after it
CURRENT HEAD: Phase 2B checkpoint commit (run `git rev-parse HEAD`; the tracked document cannot contain its own commit hash)
...
KNOWN TEST FAILURES: 17 deterministic standard Playwright failures and 6 real-backend QA failures from the Phase 0 matrix remain unrerun as a whole and are owned by later phases; Phase 2B's 29-case link-preview suite, 72-case shared-boundary suite, 175-case dependent matrix, complete uncontended 6,637-case backend suite, TypeScript, ESLint, 1,138-case frontend unit suite, and production build are green; whole-tree Ruff retains unrelated baseline findings while every changed Python file passes; one overlapping pytest run during this session produced 158 shared-test-state failures that vanished on an uncontended rerun — never run a second pytest against the shared test database
COMMANDS TO RESUME: see "Phase 2B atomic checkpoint" and "Important commands"
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; docs/PRODUCTION_READINESS_OUTBOUND_FETCH.md; backend/app/services/safe_outbound_fetch.py; backend/app/services/profile_service.py; backend/app/services/link_preview_service.py; backend/tests/test_link_preview.py; app/api/profile/organization-identity/route.ts; lib/youtubeIdentity.ts
