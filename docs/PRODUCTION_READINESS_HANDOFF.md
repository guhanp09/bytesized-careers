...
LAST COMPLETED ATOMIC SLICE:13L / CORRECT-001 — direct recruiter and represented-agency public-profile entry links now follow the real identity contract from job cards, detail pages, Inbox and Pipeline. Existing deterministic persona identities use the canonical backend enums, including on reseed of old local/staging fixtures. Two code commits: `8c8e91a` and `1fcbea2`; this handoff's checkpoint commit is the current HEAD.
NEXT ATOMIC SLICE:13M / CORRECT-002 — inventory Pipeline-to-chat, conversation lifecycle, two-persona delivery and duplicate-delivery behavior before editing. This is a high-risk multi-surface phase; do not start unless there is time to finish its tests, commit and clean handoff.
...
IMPORTANT NEW ARCHITECTURE (13L): job cards distinguish a direct recruiter's internal channel/profile link from an agency's external represented-channel link and internal `Managed by` agency profile link. Application snapshots carry the agency slug, and sent Inbox/Pipeline labels identify the agency as counterparty rather than the represented channel or job title. Deterministic persona identities now use the live schema enums; repeat seeding repairs only known legacy enum spellings on stable persona-owned IDs, preserving verification state and independently edited fields. No migration, provider, environment variable, service, dependency or AI contract changed.
...
CURRENT ALEMBIC CURRENT: disposable SQLite `.local-data/readiness-3o-backend.db` unstamped on 2026-09-23; one head 0070_activity_page_indexes; no migration in 13L. Historical PostgreSQL migration proof was not rerun.
...
KNOWN TEST FAILURES:13L final full Node1362/1362, TSC exit0, changed ESLint exit0 with2 pre-existing `no-img-element` warnings, affected standard browser15/15 profile plus61/61 workspace, and real-backend public-profile QA1/1. Backend affected suite68/68, Ruff check and `git diff --check` pass. A voluntary Ruff format check still reports three files, and each reported file also fails the same check at committed baseline; do not reformat unrelated code merely to clear that baseline warning. The QA server logged the known intermittent `destination stream closed early` disconnect without failing the test. No task-caused failure remains. 13E504/504 is the latest whole standard aggregate; full QA was not rerun in13L. Backend last full3W7712passed+65skipped=7777. External provider/infrastructure/manual/legal gates remain; NO-GO.
COMMANDS TO RESUME:`git status --short`; `git branch --show-current`; `git rev-parse HEAD`; verify five frozen refs; read 13L checkpoint and ledger CORRECT-002. Inventory Pipeline-to-chat and two-persona paths before editing. Run focused browser and backend messaging tests on disposable local state only; do not contact hosted databases or providers.
FILES TO READ FIRST:13L checkpoint; ledger CORRECT-002; `components/you/PipelineBoard.tsx`, `components/you/ApplicationsWorkspace.tsx`, `components/you/CompactChatDock.tsx`, `lib/applicationPipeline.ts`, `lib/messaging.ts`, `tests/e2e/applications-pipeline.spec.ts`, `tests/e2e/messaging-regressions.spec.ts`, `tests/e2e/qa/workspace-workflow-audit.spec.ts`, `backend/tests/test_messaging.py`. Do not edit unrelated IDE PHP files.
...
## Phase13L checkpoint — certify recruiter and agency public-profile paths (2026-09-23)

```text
Phase:13L / CORRECT-001
Status:COMPLETE / VALIDATED locally
Initial HEAD:8b3d8bb00455d282f90c096315d538b147a107f6
Final HEAD:the Phase13L documentation checkpoint commit containing this section; resolve with
 `git rev-parse HEAD` on the integration branch. Final code HEAD before docs:1fcbea2.
Commit(s):8c8e91ac78709a36e954c1741144e15b9db199c5
 `fix(profiles): connect recruiter and agency entry links`;
 1fcbea2 `fix(seed): reconcile legacy persona identity enums`; this documentation checkpoint.
Files materially changed:`components/JobCard.tsx`, `lib/ownerInteractions.ts`,
 `lib/applicationPipeline.ts`, `components/you/ApplicationsWorkspace.tsx`,
 `components/you/PipelineBoard.tsx`, `backend/app/db/seed_data_personas.py`,
 `backend/app/db/seed.py`, focused Node/backend/standard browser tests,
 `tests/e2e/qa/public-profile-navigation.spec.ts`, execution ledger and handoff.
Migrations:none. One Alembic head0070_activity_page_indexes; explicit read-only current on
 disposable SQLite `.local-data/readiness-3o-backend.db` is unstamped. PostgreSQL migration
 harness not rerun because no schema changed and the real local service is unavailable.
Behavior changed:for a direct job, the public card's channel label resolves to the recruiter's
 internal public profile even when the channel has an external URL. For a represented agency job,
 the channel label stays external while `Managed by` resolves to the internal agency profile;
 its nested click no longer triggers card navigation. Application snapshots preserve agency slug;
 sent Inbox and Pipeline identify/link the agency as counterparty while retaining the job title
 as context. Direct and agency deterministic fixtures now use canonical hiring-identity enum
 values; reseeding old fixtures repairs only known legacy spellings on stable persona IDs and
 does not reset verification state or independently edited fields.
Security assumptions:public profiles and agency rails are loaded from real backend identity
 records in Backend mode; represented-channel verification still decides public rail visibility.
 External channel URLs are never treated as internal identity authority. Seed repair is limited
 to development/test/staging persona IDs and cannot reassign a customer identity. The QA test
 uses disposable SQLite and cleans its created application/jobs. No auth/session/AI boundary changed.
Tests run:full frontend Node; TypeScript; changed-file ESLint; standard Chromium profile and
 workspace suites; one new real-backend QA multi-persona path; backend persona/profile/job/QA/
 seed suites; Ruff lint; `git diff --check`; Alembic heads/current on disposable SQLite.
Exact results:Node1362/1362; TSC exit0; ESLint exit0 with2 existing image warnings;
 standard profile15/15 and affected Applications/Pipeline61/61; real-backend QA1/1;
 backend affected68/68; Ruff check passed; diff check passed; one Alembic head and no
 current revision on the explicit disposable SQLite. The fresh QA Next build passed.
Known test failures:the QA server emitted an intermittent `destination stream closed early`
 log but the exact assertion and run passed. Ruff format --check is not a gate here and
 reports three pre-existing unformatted files, each also failing against its committed
 baseline; task-added seed lines were made formatter-compatible. No unresolved task-caused
 compile, test, security or browser failure. Complete standard/QA/backend aggregates were
 not rerun in this slice; historical aggregates remain in prior checkpoints.
Known external failures:no hosted database, identity provider, managed Redis/storage/email,
 deployment, domain, alert destination, legal counsel or live accessibility review was used.
Remaining risks:CORRECT-002/003/004/005/006/008 and all other nonvalidated ledger rows;
 general browser bearer AUTH-007; durable paid AI worker/budget gates; live provider and
 infrastructure drills, production credential/config validation, restore and staging soak.
 AI Import is retained, gated and draft-only; no AI behavior changed. Release:NO-GO.
Next phase:13M / CORRECT-002 — Pipeline-to-chat and two-persona messaging lifecycle.
Important commands:`git status --short`; `git branch --show-current`; `git rev-parse HEAD`;
 `git show-ref` for all five frozen refs; read the CORRECT-002 ledger row and named files above;
 run focused Playwright and backend messaging tests on disposable local state. No push,
 deployment or hosted Neon/Vercel/Render modification occurred in this slice.
```
...
```text
Phase:13F / PRODUCT-004A (plus notification deep-link defect found by validation)
Status:COMPLETE atomic slice; PRODUCT-004 remains IN_PROGRESS for the wider seed/demo inventory.
Initial HEAD:796b2de96e2132f0bdb41dcc6312f8913f8e06be
Final HEAD / Commit:git log -1 --format=%H --grep='fix(talent): keep backend portfolios grounded'
Files materially changed:app/talent/[id]/page.tsx; backend marketplace create route;
 backend/scripts/qa_talent_portfolio_cleanup.py; existing devDataSource unit,
 candidate-surface real-backend QA and marketplace core test; ledger/handoff.
Behavior changed:the talent detail portfolio loader now chooses synthetic work only when
 MarketplaceDataSourceState.source is mock. Production/default backend mode with no override
 cookie reads the owner's real public, published portfolio and keeps only IDs selected by the
 listing, in listing order. Backend absence/outage returns no portfolio rather than fake work.
 Explicit local mock mode and a valid test mock-cookie override retain canonical demo samples.
A defect found during real QA is fixed in the same listing transaction: create_talent_listing
 flushes the new row before dispatching its creation notification, so resource_id/action_url use
 the assigned UUID instead of the persisted strings None and /talent/None.
Baseline / non-vacuity:read-only execution of the actual loader on796b2de with production
 source=backend,overrideSource=null returned [{id:synthetic}] and made zero backend reads
 (/tmp/creatorjobs-13f-portfolio-baseline-20260922.log). Added unit set initially9pass/4fail.
 The extended existing backend test initially fails resource_id 'None' != listing UUID
 (/tmp/creatorjobs-13f-notification-baseline-20260922.log).
Test harness:realQA creates two real public portfolio rows and one published talent listing via
 authenticated APIs, views it anonymously with no cj_data_source cookie, proves only the selected
 owned title appears and known synthetic copy does not, then exercises normal API deletion.
 A guarded helper opens only APP_ENV=test plus the exact disposable SQLite URL in read/write-
 existing mode, refuses symlinks/other URLs/environments, and hard-removes only the random UUID
 namespace plus its notification. It never resets the corpus. Final database check:0 owned
 portfolio rows,0 owned listing rows,0 invalid /talent/None notifications.
Tests / exact results:devDataSource13/13; full Node1328/1328; TSC0; changed ESLint0;
 changed backend Ruff0; focused backend notification1/1 after baseline1/1 failure; selected
 marketplace/notification backend files exit0 (no exact count claimed without JUnit+collection);
 final realQA candidate surface10/10,28.7s,1worker, fresh production build0.
 Initial realQA8pass/1fail correctly enforced the existing published-portfolio cover requirement;
 fixture added its local owned cover and then9/9. Final guard/cleanup additions produce10/10.
Evidence:/tmp/creatorjobs-13f-{unit-baseline,focus,node-final,tsc-final,lint-final,
 qa,qa-final,qa-clean-final,qa-notification-final,notification-baseline,notification-final,
 backend-affected,ruff-final,portfolio-baseline}-20260922.log.
Migrations / dependencies / environment variables / services:none.
Security assumptions:all mutation is confined to loopback, explicit disposable QA SQLite and
 random namespace. No hosted data/credentials/providers. Cleanup refuses remote PostgreSQL even
 before opening a database. Provider keys blank; live AI smoke exact0. AI behavior unchanged.
LAST_FULL_SUITE_OBSERVED:Node1328 current; standard13E504/504; fullQA13C293/293 includes292
 deterministic plus unintended live smoke before repaired gate (read13C correction); a11y87/87.
 Backend3W full7712passed+65skipped=7777JUnit cases, exit0, collection7777.
EXPECTED_CURRENT_COLLECTION:Node1328; standard504; realQA298 (+2); backend7777 unchanged.
Current Alembic head/current:0070_activity_page_indexes single; last recorded disposable SQLite
 current unstamped Sept17. No migration; PostgreSQL remains unavailable and was not re-probed.
Known external failures / remaining risks:PRODUCT-004 wider production demo isolation is not
 certified by this one surface. All hosted/provider/legal/manual/restore/soak gates remain.
 Queue-backed AI remains unresolved; AI is retained and draft-only. Release assessment:NO-GO.
Next phase:13G / TRUST-001. Reproduced source-level issue:both talent card and detail rateLabel
 replace stored USD/dollar data with hard-coded role-derived rupee prices. Build one canonical
 truthful formatter, preserve exact source currency/range/note and cover flexible/unknown/unpaid
 states without inventing amounts. Inspect job compensation paths before calling the row closed.
Frozen refs:all five exact hashes unchanged at pre-commit verification; verify again after commit.
No push/deployment; hosted Neon/Vercel/Render untouched.
```
