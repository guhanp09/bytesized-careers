...
CURRENT PHASE: Phase 3 resumed hardening inside Phase 13 certification — RATE-003 endpoint quotas
LAST COMPLETED ATOMIC SLICE: Phase 3K — the backend half of RATE-003. One reusable dependency now distinguishes trusted caller-IP buckets from independently verified user buckets, and every policy is inspectable from the FastAPI dependency graph. Verification/refresh, public search, media upload, outbound preview/fetch, messaging mutations, admin/support, job/import/review/marketplace actions and checkout now have the intended shared Redis identity and category boundary. Logout/logout-all remain deliberately reachable during compromise recovery. The 48-case quota/identity focus and 389-case affected-route matrix are green. RATE-003 remains IN_PROGRESS only because the two Next-runtime Google Places routes still bypass this backend boundary. The checkpoint commit is named `security(rate-limit): enforce user-scoped endpoint quotas`; resolve the exact current HEAD with `git rev-parse HEAD` because this handoff is committed with the implementation.
NEXT ATOMIC SLICE: Phase 3L / OF-105: move Google Places autocomplete/details behind a bounded authenticated backend provider boundary (or an equivalently shared server contract), with input ceilings, redirect refusal, decoded JSON/body and total-time bounds, Redis user quotas, privacy-safe errors and route tests. Do not use a process-local Next limiter. The host remains abnormally loaded (about 54 at checkpoint), so do not start Playwright until it is quiet. Gitleaks and a built-image scan still require an environment with those tools and a functioning Docker daemon.
...
NEW ENVIRONMENT VARIABLES: none in RATE-003A. Production still uses the existing `RATE_LIMIT_BACKEND=redis` and secret `REDIS_URL`; the test-only real-service command requires `RATE_LIMIT_TEST_REDIS_URL` and refuses anything except loopback. Phase 12E's backend-only planned-overlap `GOOGLE_OAUTH_EXCHANGE_PREVIOUS_SECRET` and `EMAIL_WEBHOOK_PREVIOUS_SECRET` remain as documented, along with the earlier auth/media/realtime/AI settings in `backend/app/core/config.py`, `.env.example` and `DEPLOYMENT.md`.
...
NEW SERVICES: production now concretely requires a managed Redis 7.2+ service for shared rate limiting, but none was provisioned or contacted. Local validation used verified official Redis 7.4.11 source bound only to loopback; CI pins `redis:7.4.11-alpine`. The test process was stopped and the temporary source/build was moved recoverably to Trash. Existing local/CI audit, rotation, incident, metric, error and synthetic services remain as documented.
...
KNOWN TEST FAILURES: no RATE-003A task-caused failure remains. New quota/identity focus: 48/48. Affected auth/admin/profile/search/marketplace/messaging/review/import matrix: 389/389. Changed-file Ruff and diff checks pass. The authoritative full-backend result from the immediately preceding RATE-001 checkpoint remains 7,499 passed/65 skipped/71 warnings; this slice did not spend another 52-minute full run on the load-54 host. Frontend Node remains 1,292/1,292. Real Redis remains exactly 17 allowed/183 rejected twice. Production Python audit remains one exact analysed structurally unreachable ecdsa residual and zero unanalysed. TypeScript/build remain green; ESLint has 0 errors/32 existing warnings; PostgreSQL remains 29/29 from Phase 13A. Browser status is unchanged: accessibility 87/87, synthetics 6/6, standard 502 passed/2 load timeouts then exact cases green, and QA 256 passed/35 host-contended failures/1 skipped with 22 focused cases cleared. Eight late persona timeouts still require quiet-host isolation. No timeout/sleep/assertion was relaxed.
COMMANDS TO RESUME: `git status --short`; `git branch --show-current`; `git rev-parse HEAD`; `uptime`. RATE-003A focus: `cd backend && APP_ENV=test .venv/bin/python -m pytest -q tests/test_endpoint_rate_limits.py tests/test_rate_limit.py tests/test_trusted_client_identity.py`. OF-105 inventory: `rg -n 'places.googleapis.com|location/autocomplete|location/details' app backend tests`; inspect the two Next routes, existing backend fixed-provider clients and session proxy patterns before choosing the boundary. Do not start Playwright while load remains abnormal. On a quiet host run `npm run test:e2e:qa` alone and retain the exact final report. Gitleaks and built-image scanning require another environment.
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; the Phase 3K checkpoint below; docs/PRODUCTION_READINESS_OUTBOUND_FETCH.md; backend/app/core/rate_limit.py; backend/app/api/deps.py; backend/tests/test_endpoint_rate_limits.py; `app/api/location/autocomplete/route.ts`; `app/api/location/details/route.ts`; then the existing Next-to-backend authenticated proxy patterns
