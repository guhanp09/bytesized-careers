...
CURRENT PHASE: Phase 13 — final release certification
LAST COMPLETED ATOMIC SLICE: Phase 13B — browser/security certification repair and triage. Three-engine accessibility is 87/87 and the six owned synthetics are 6/6 in single serial aggregates. A real Star persistence race, duplicate realtime reconnect reconciliation, invented third-party demo avatars, five browser-test record/state assumptions, and the backend production-audit gate were repaired. Frontend Node is 1,291/1,291; TypeScript/build/Ruff are green; ESLint has 0 errors/32 existing warnings; focused backend is 12/12; frontend audit has 0 high/critical findings; the production-only Python audit has one exact analysed, structurally unreachable ecdsa finding and zero unanalysed findings. The standard aggregate was 502 passed/2 host timeouts, each exact case then passed. The QA aggregate was 256 passed/35 host-contended failures/1 skipped over 1.9h; 22 failed cases passed focused unchanged, five reproduced only time budgets while the host load was 32–57, and eight late timeout cases remain to isolate on a quiet host. CERT-001 remains IN_PROGRESS. The checkpoint commit is named `fix(certification): harden browser and dependency gates`; resolve the exact current HEAD with `git rev-parse HEAD` because this handoff is committed with the repair.
NEXT ATOMIC SLICE: Phase 13C — on a quiet host, rerun the complete QA aggregate first, then isolate only any remaining failures. The known Shine desktop fixture is aborted by its existing 30-second client budget while the local backend is starved; the representative Hired journey completed hire/reload/cross-persona/Pipeline checks and timed out only at its final logout/relogin. Do not increase timeouts, add sleeps, skip tests, or weaken assertions. After QA is green, run Gitleaks and a built-image scan in an environment where those tools and a Docker daemon exist.
...
NEW ENVIRONMENT VARIABLES: Phase 12E adds backend-only planned-overlap `GOOGLE_OAUTH_EXCHANGE_PREVIOUS_SECRET` and `EMAIL_WEBHOOK_PREVIOUS_SECRET`; neither is set during compromise containment and both default unset. Production frontend now enforces the existing server-only `GOOGLE_OAUTH_EXCHANGE_SECRET` rather than silently omitting it. Earlier variables remain: optional frontend/build `NEXT_PUBLIC_RELEASE_SHA`; release-build `CREATORJOBS_RELEASE` (Vercel may supply `VERCEL_GIT_COMMIT_SHA`), `CREATORJOBS_PRIVATE_SOURCE_MAPS`, optional `CREATORJOBS_SOURCE_MAP_DIR` and `CREATORJOBS_NEXT_BUILD_DIR`; backend GOOGLE_CLIENT_ID; backend GOOGLE_CLIENT_SECRET; GOOGLE_OAUTH_EXCHANGE_SECRET shared only between NextAuth and FastAPI; OAUTH_CREDENTIAL_KEYS; OAUTH_CREDENTIAL_ACTIVE_KEY_ID; OAUTH_CREDENTIAL_WRITE_MODE; ALLOW_OAUTH_PLAINTEXT_COMPATIBILITY_IN_PRODUCTION; AUTH_SESSION_MODE; ALLOW_LEGACY_REFRESH_COMPATIBILITY_IN_PRODUCTION; REFRESH_REUSE_GRACE_SECONDS; ADMIN_STRONG_AUTH_REQUIRED; ADMIN_STRONG_AUTH_MAX_AGE_MINUTES; STRONG_AUTH_SECRET_KEYS; STRONG_AUTH_SECRET_ACTIVE_KEY_ID; INVITE_ONLY_BETA; request/email/media/realtime/AI settings already documented in backend/app/core/config.py, `.env.example` and `DEPLOYMENT.md`.
NEW DEPENDENCIES: none in Phase 13B, 13A, 12E, 12D, 12C, 12B or 12A; pip-audit 2.10.1 is a pinned ephemeral audit tool, not a shipped dependency; direct exact frontend `axe-core` 4.13.0 remains the Phase 11L release-gate dependency; backend still declares its already-locked runtime `httpx==0.28.1` and `httpcore==1.0.9` usage directly
NEW SERVICES: no new deployed service in Phase 13B, 13A or 12E; new local/CI audit command `python -m scripts.audit_production_dependencies`; new operator command `scripts.rotate_strong_auth_secrets`; planned shared-secret overlap settings; enhanced operator command `scripts.grant_admin --revoke`; machine-checked credential/incident/tabletop contracts; finite alert/metric/error/synthetic services already listed in prior checkpoints
OUTSTANDING EXTERNAL REQUIREMENTS: a human/operator tabletop; immutable artifact rollback/traffic-shift drill; hosted database/PITR and media restore; real provider outage/failover/revocation drills; production credential rotation; production log ingestion/retention/access; real alert-destination delivery and acknowledgement; standalone-worker absence/process-death monitoring; scheduled synthetics against isolated staging data; evidence-backed traffic/latency/capacity thresholds; authenticated GitHub fetch/protection inspection; matching production GOOGLE_OAUTH_EXCHANGE_SECRET provisioning; real Google consent-screen scope configuration/verification and live login/incremental-consent/reconnect/refresh/revoke/outage drill; real OAuth/strong-auth keyring provisioning plus rotation drills; hosted credential backfill/encrypted-only verification; a physical authenticator-device drill and lost-all-factors support procedure; email DNS/provider; managed Postgres/Redis/storage; counsel approval; accessibility review; staging soak
KNOWN TEST FAILURES: no Phase 13B task-caused static/unit/build/security failure remains. Whole-backend Ruff exit 0; last complete backend aggregate remains 7,476 passed/65 skipped/71 warnings from Phase 13A; changed backend focus is 12/12; frontend Node 1,291/1,291; TypeScript exit 0; ESLint 0 errors/32 existing warnings; production build exit 0 with 32/32 static pages; PostgreSQL gate remains 29/29 from Phase 13A. Accessibility is now a canonical 87/87 aggregate and synthetics 6/6. Standard Playwright was 502 passed/2 failed under host load; both exact unchanged tests passed, the reference timestamp repeated 10/10, and a broader affected set passed 45/45. QA was 256 passed/35 failed/1 skipped over 1.9h while system load was roughly 49–57 and Next repeatedly reported closed destination streams. Focused unchanged reruns cleared 22 of those 35. Shine desktop remains reproducibly aborted at the local fixture request's existing 30-second budget while the host is saturated (mobile passes); four grouped persona tests hit the global 45-second budget, and the exact Hired rerun completed every product assertion through cross-persona/Pipeline before timing out on final logout/relogin. Eight late persona timeouts remain to isolate on a quiet host. No timeout/sleep/assertion was relaxed.
COMMANDS TO RESUME: `git status --short`; `git branch --show-current`; `git rev-parse HEAD`; `uptime`. Do not start Playwright while load remains abnormal. On a quiet host run `npm run test:e2e:qa` first and retain the exact final report; isolate any remaining case with `npx playwright test -c playwright.qa.config.ts <file> --workers=1 --grep '<exact title>'`. Do not run a build or another Playwright suite concurrently against `.next`. Already-green local gates need not be repeated unless code changes: `npm run test:e2e:a11y` (87/87), `npm run test:e2e:synthetics` (6/6), `npm run audit:production`, `cd backend && .venv/bin/python -m scripts.audit_production_dependencies`. Gitleaks and built-image scanning require another environment.
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; the Phase 13B checkpoint below; tests/e2e/qa/qa-personas.spec.ts; tests/e2e/qa/draft-assistant.spec.ts; components/you/ApplicationsWorkspace.tsx; lib/realtimeMessaging.ts; backend/scripts/audit_production_dependencies.py; `.github/workflows/security.yml`
