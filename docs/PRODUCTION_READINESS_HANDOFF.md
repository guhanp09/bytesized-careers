...
LAST COMPLETED ATOMIC SLICE: Phase 12D — CreatorJobs now has specific incident paths and a machine-checked no-provider tabletop for rollback, bad migration, customer/admin compromise, database/media loss, abuse/rate-limit failure, email, AI, realtime and Google/YouTube incidents. The implementation commit is named `ops(runbooks): make incident containment executable`; resolve the exact current HEAD with `git rev-parse HEAD` because this handoff is committed with the implementation. OPS-006 is VALIDATED locally; every hosted/human/provider/restore drill is still named external.
NEXT ATOMIC SLICE: Phase 12E — OPS-007 credential-rotation inventory and local rehearsal. Inventory every credential/keyring first, distinguish dual-key expand/rewrap/retire flows from single-secret global logout/redeploy flows, use only disposable local data, and never rotate a live credential.
...
KNOWN TEST FAILURES: no Phase 12D task-caused failure remains: runbook/tabletop contract 5/5, emergency grant/revoke 2/2, grant/revoke plus audit integrity 15/15, configuration/operator-contract focus 120 passed/1 skipped, cross-subsystem incident matrix 281/281, changed Python Ruff exit 0. Expected current collections are backend 7,530 and frontend node 1,284; the last complete observed aggregates remain backend 7,506/exit 0 and frontend 1,276/1,276 from Phase 12A, so Phase 13 owes the deltas rather than inferring them from focused runs. Phase 12C browser truth is unchanged: selector contract 3/3/listing 6/6, but two real-backend synthetic attempts under unrelated host load 50–76 ended 4 passed/2 timed out and 2 passed/4 timed out at different unchanged points. OPS-005 stays IMPLEMENTED until a quiet-host 6/6. Whole-backend Ruff remains a known local Phase 13 blocker with 67 findings outside the Phase 12A–D touched files. Phase 11L accessibility evidence remains canonical 85/87 with each exact Firefox host timeout green isolated, and no standard aggregate 504/504 claim until Phase 13.
COMMANDS TO RESUME: `git status --short`; `git branch --show-current`; `git rev-parse HEAD`; `.venv/bin/python -m alembic heads` from backend; read the Phase 12D checkpoint and inventory every production credential declaration/enforcement/consumer before OPS-007. Start with OAuth and strong-auth dual keyrings, JWT/NextAuth single secrets, Google exchange/client secrets, email webhook/SMTP, OpenAI, Redis/database, media and deployment credentials. Use disposable local values only. On a quiet host, run `npm run test:e2e:synthetics` before promoting OPS-005. Never run two pytest processes against the shared test database or build concurrently with Playwright against `.next`.
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; this Phase 12D checkpoint; docs/INCIDENT_RESPONSE.md; docs/INCIDENT_TABLETOP.md; backend/app/core/config_contract.py; backend/app/core/oauth_credentials.py; backend/app/core/strong_auth_secrets.py; backend/scripts/rotate_oauth_credentials.py; backend/scripts/grant_admin.py; frontend auth/secret configuration validation
