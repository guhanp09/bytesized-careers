...
LAST COMPLETED ATOMIC SLICE: Phase 12A — browser and backend failures now converge on a privacy-bounded structured-stdout contract, with fail-safe ingestion and private release source maps. The implementation commit is named `ops(errors): add privacy-bounded failure reporting`; resolve the exact current HEAD with `git rev-parse HEAD` because this handoff is committed with the implementation.
NEXT ATOMIC SLICE: Phase 12B — implement the smallest bounded-cardinality OPS-004 operational telemetry seam for HTTP/errors and the ledger-named DB, Redis, email, realtime, upload, auth and AI boundaries. Survey existing counters/health/events first; do not build a product-analytics system or make tracing-provider configuration a local boot dependency.
...
NEW DEPENDENCIES: none in Phase 12A; direct exact frontend `axe-core` 4.13.0 remains the Phase 11L release-gate dependency; backend still declares its already-locked runtime `httpx==0.28.1` and `httpcore==1.0.9` usage directly
NEW SERVICES: privacy-bounded `/api/v1/telemetry/client-errors` ingestion and `app.core.error_reporting`; private release-map collector `scripts/collect-private-source-maps.mjs`; app.services.activity_pagination deterministic activity-feed boundary; app.services.safe_outbound_fetch shared public-URL boundary; docs/PRODUCTION_READINESS_OUTBOUND_FETCH.md complete caller inventory; plus all previously documented OAuth/session/strong-auth services
...
KNOWN TEST FAILURES: no task-caused failure is open. Phase 12A: frontend node 1,276/1,276; focused error reporting 6/6 frontend and 5/5 backend; TypeScript and changed-file Ruff green; exact release build archived 454 maps, stripped 73 public maps, and left zero public maps/hints. The first full backend aggregate reached 100% with only two time-sensitive brand-enrichment assertions failing after the run exceeded their three-minute liveness fixture; both passed 2/2 fresh, and the complete boundary class passes 11/11 after its existing injectable clock was actually supplied. The authoritative rerun collected 7,506 tests and exited 0. Complete ESLint exits 0 with 32 existing warnings. Whole-backend Ruff remains a known local Phase 13 blocker with 67 findings, all in files untouched by Phase 12A; changed-file Ruff is green and those unrelated findings were not opportunistically rewritten. Phase 11L browser evidence remains unchanged: canonical accessibility 85/87 with each exact Firefox host timeout green isolated, and no canonical 504/504 standard aggregate claim until Phase 13.
COMMANDS TO RESUME: `git status --short`; `git branch --show-current`; `git rev-parse HEAD`; `.venv/bin/python -m alembic heads` from backend; then inventory the existing health/event/provider/worker seams before implementing OPS-004. Never run two pytest processes against the shared test database, never label telemetry with customer input, and never run Next build concurrently with Playwright against `.next`.
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; this Phase 12A checkpoint; backend/app/core/logging.py; backend/app/core/error_reporting.py; backend/app/health/router.py; backend/app/health/service.py; backend/app/core/rate_limit.py; backend/app/notifications/worker.py; backend/app/services/job_import_processing_service.py; backend/app/realtime; media write services; deployment/runbook docs
