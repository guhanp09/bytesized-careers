...
LAST COMPLETED ATOMIC SLICE: Phase 2A — shared DNS-pinned outbound-fetch boundary plus job/brand caller migration
NEXT ATOMIC SLICE: Phase 2B — migrate portfolio HTML and oEmbed previews to the shared boundary with fixed-provider JSON policy and graceful fallback parity
CURRENT HEAD: Phase 2A checkpoint commit (run `git rev-parse HEAD`; the tracked document cannot contain its own commit hash)
...
KNOWN TEST FAILURES: 17 deterministic standard Playwright failures and 6 real-backend QA failures from the Phase 0 matrix remain unrerun as a whole and are owned by later phases; Phase 2A's 72-case security suite, 327-case dependent matrix, complete 6,614-case backend suite, frontend gates, build, and four-case import browser suite are green; whole-tree Ruff retains unrelated baseline findings while every changed Python file passes
COMMANDS TO RESUME: see "Phase 2A atomic checkpoint" and "Important commands"
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; docs/PRODUCTION_READINESS_OUTBOUND_FETCH.md; backend/app/services/safe_outbound_fetch.py; backend/app/services/link_preview_service.py; backend/tests/test_safe_outbound_fetch.py; backend/tests/test_link_preview.py; backend/app/services/job_url_fetcher.py; tests/jobUrlImportFlow.test.mjs
