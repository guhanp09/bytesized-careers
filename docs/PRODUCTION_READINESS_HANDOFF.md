...
LAST COMPLETED ATOMIC SLICE: Phase 12C — the finite privacy-safe error/metric stream now has one executable provider-neutral alert policy, specific response guidance, and a six-journey no-cost synthetic subset of the existing real-backend QA harness. The implementation commit is named `ops(alerts): add finite policies and local synthetics`; resolve the exact current HEAD with `git rev-parse HEAD` because this handoff is committed with the implementation. OPS-005 remains IMPLEMENTED, not VALIDATED, until the six journeys pass together on a quiet host and live delivery is proven externally.
NEXT ATOMIC SLICE: Phase 12D — OPS-006 specific incident runbooks and local tabletop checks. Reuse the alert/health/config/recovery contracts already present; do not claim a hosted drill, backup restore, legal approval or provider action that did not occur.
...
NEW DEPENDENCIES: none in Phase 12C, 12B or 12A; direct exact frontend `axe-core` 4.13.0 remains the Phase 11L release-gate dependency; backend still declares its already-locked runtime `httpx==0.28.1` and `httpcore==1.0.9` usage directly
NEW SERVICES: finite alert policy `app.core.operational_alerts`; JSONL operator/adapter entry point `scripts.evaluate_operational_alerts`; tagged canonical real-backend synthetic entry point `npm run test:e2e:synthetics`; finite structured-stdout `metric_event` producers in `app.core.operational_metrics` and request middleware; privacy-bounded `/api/v1/telemetry/client-errors` ingestion and `app.core.error_reporting`; private release-map collector `scripts/collect-private-source-maps.mjs`; app.services.activity_pagination deterministic activity-feed boundary; app.services.safe_outbound_fetch shared public-URL boundary; docs/PRODUCTION_READINESS_OUTBOUND_FETCH.md complete caller inventory; plus all previously documented OAuth/session/strong-auth services
OUTSTANDING EXTERNAL REQUIREMENTS: production log ingestion/retention/access; real alert-destination delivery and acknowledgement; standalone-worker absence/process-death monitoring; scheduled synthetics against isolated staging data; evidence-backed traffic/latency/capacity thresholds; authenticated GitHub fetch/protection inspection; matching production GOOGLE_OAUTH_EXCHANGE_SECRET provisioning; real Google consent-screen scope configuration/verification and live login/incremental-consent/reconnect/refresh/revoke/outage drill; real OAuth/strong-auth keyring provisioning plus rotation drills; hosted credential backfill/encrypted-only verification; a physical authenticator-device drill and lost-all-factors support procedure; email DNS/provider; managed Postgres/Redis/storage; counsel approval; accessibility review; backup/restore; staging soak
KNOWN TEST FAILURES: no task-caused assertion, timeout or product regression is accepted as green. Phase 12C local alert policy 8/8, synthetic selector contract 3/3 and selector listing 6/6 pass. Two real-backend synthetic attempts were run while unrelated long-lived editor processes drove host load from roughly 50 to 76. Attempt one used initially broader existing apply/message journeys: publish/browse/auth/admin passed, apply/message hit unchanged 45-second ceilings (4 passed, 2 failed in 5.3m). Tags were then moved to narrower existing product contracts without changing any test body/assertion/timeout. Attempt two passed browse/auth; apply reached its final session read after UI submission, publish timed out after having passed attempt one, admin timed out during login after passing attempt one, and message did not complete its unchanged 12-second send expectation (2 passed, 4 failed in 6.6m). The variable tests and large timing swing are recorded as environmental, but OPS-005 remains only IMPLEMENTED until `npm run test:e2e:synthetics` passes 6/6 on a quiet host. Phase 12B's latest complete backend baseline remains Phase 12A's 7,506/exit-0 run; Phase 13 owes the now-larger aggregate. Whole-backend Ruff remains a known local Phase 13 blocker with 67 findings outside the Phase 12A/B/C touched files. Phase 11L browser evidence remains unchanged: canonical accessibility 85/87 with each exact Firefox host timeout green isolated, and no canonical 504/504 standard aggregate claim until Phase 13.
COMMANDS TO RESUME: `git status --short`; `git branch --show-current`; `git rev-parse HEAD`; `.venv/bin/python -m alembic heads` from backend; read the Phase 12C checkpoint and begin OPS-006 by inventorying existing rollback, compromise, data-loss, abuse and provider-specific recovery material. On a quiet host, run `npm run test:e2e:synthetics` before promoting OPS-005. Never run two pytest processes against the shared test database, never label telemetry with customer input, and never run Next build concurrently with Playwright against `.next`.
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; this Phase 12C checkpoint; docs/OPERATIONS_ALERTS.md; backend/app/core/operational_alerts.py; backend/scripts/evaluate_operational_alerts.py; backend/tests/test_operational_alerts.py; existing rollback/recovery/security runbooks and deployment docs
