...
LAST COMPLETED ATOMIC SLICE: Phase 3N / RATE-004A — bounded HTTP admission per ASGI worker, immediate safe 503 overload, cancellation/response-cleanup slot ownership, one separately bounded liveness slot. Production explicitly supplies MAX_CONCURRENT_HTTP_REQUESTS; local capacity is not a production recommendation. Complete backend and affected real-backend browser validation passed. Commit: `security(request): bound in-flight HTTP admission` (this document is committed with the slice).
NEXT ATOMIC SLICE: Phase 3O / RATE-004 — bound slow inbound body receive time without wrapping transactions in a blanket cancellation deadline. Inspect RequestBodyLimitMiddleware and FastAPI body-error translation before implementing. Full handler/DB statement deadlines and ingress protection still need explicit contracts. RATE-005 must then be reconciled with existing Phase 6 implementation, not duplicated.
...
CURRENT ALEMBIC CURRENT: explicitly disposable local SQLite `.local-data/readiness-3n-backend.db` is unstamped (2026-09-15); one head 0070_activity_page_indexes; no migration created. Historical PostgreSQL proof remains separate and was not rerun for middleware-only work.
...
KNOWN TEST FAILURES: Phase 3N full backend JUnit: 7,596 passed / 65 skipped / 0 failures / 0 errors / 72 warnings, 7,661 total; successful fresh collection independently confirms 7,661. Focus: 195 passed/1 existing skip; 15 new admission cases. Node aggregate 1,297/1,297; whole-backend Ruff clean. Real-backend browser matrix 42/42 (CSP, organization resolver, all QA personas) plus successful production build. TypeScript/lint remain the Phase 3M results (0 errors; 32 existing ESLint warnings), frontend source unchanged. Existing Next destination-stream-closed diagnostics occurred despite green assertions and remain tracked, not suppressed.
COMMANDS TO RESUME: `git status --short`; `git branch --show-current`; `git rev-parse HEAD`; `uptime`. Read Phase 3N checkpoint and RATE-004 ledger. Focus: `cd backend && APP_ENV=test .venv/bin/python -m pytest -o addopts= -q tests/test_http_admission.py tests/test_request_body_limit.py tests/test_config.py tests/test_config_contract.py tests/test_health_contracts.py tests/test_operational_metrics.py`. Browser: `npm run test:e2e:qa -- tests/e2e/qa/organization-resolver.spec.ts tests/e2e/qa/qa-personas.spec.ts tests/e2e/qa/content-security-policy.spec.ts --reporter=line`. No overlapping pytest/build/browser owners; no repeated Docker/Gitleaks probes without evidence environment changed.
FILES TO READ FIRST: docs/PRODUCTION_READINESS_EXECUTION.md; Phase 3N below; backend/app/middleware/{http_admission,request_body_limit}.py; backend/tests/{test_http_admission,test_request_body_limit}.py; backend/app/main.py; backend/app/core/{config,config_contract}.py; backend/app/db/session.py.
