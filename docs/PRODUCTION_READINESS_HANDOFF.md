...
LAST COMPLETED ATOMIC SLICE: Phase 1D-2 — real TOTP credential, enrollment, challenge, recovery, and revocation backend lifecycle
NEXT ATOMIC SLICE: Phase 1D-3 — administrator TOTP enrollment/challenge/recovery browser UX and end-to-end privilege validation
CURRENT HEAD: Phase 1D-2 checkpoint commit (run `git rev-parse HEAD`; the tracked document cannot contain its own commit hash)
...
IMPORTANT NEW ARCHITECTURE: FastAPI verifies Google ID tokens and owns provider credentials; NextAuth exposes only an allowlisted session; the database enforces both OAuth ownership invariants; provider credentials and TOTP factor secrets use separate, versioned AES-256-GCM keyrings; backend password/Google logins issue durable session families with one-time hash-only refresh credentials; authenticated HTTP requests treat signed `sid` as a database-backed revocation boundary; administrator requests pass a global policy gate backed only by fresh assurance on their durable database session; the deliberately narrow base-auth boundary now owns logout and real factor ceremonies; one encrypted pending/confirmed TOTP credential is serialized per administrator, TOTP steps are one-use across instances, recovery codes are 160-bit/hash-only/one-use, failed factor proofs lock durably, and factor activation/recovery/change revokes affected sibling or all session families
...
COMMANDS TO RESUME: see "Phase 1D-2 atomic checkpoint" and "Important commands"
FILES TO READ FIRST: backend/app/services/strong_auth_service.py; backend/app/api/v1/routers/auth.py; backend/app/schemas/strong_auth.py; backend/app/core/strong_auth_secrets.py; backend/app/core/totp.py; backend/app/core/auth_assurance.py; backend/app/api/deps.py; backend/app/models/strong_auth.py; backend/alembic/versions/0058_strong_auth_totp.py; backend/tests/test_strong_auth_lifecycle.py; backend/tests/test_strong_auth_postgres.py; lib/backend.ts; lib/auth.ts; components/settings/SettingsClient.tsx; app/admin/layout.tsx
