...
| CR-08 | `REDIS_URL` | Redis provider | backend config/boot choice → startup ping and bounded Redis client → one-operation shared rate limiter | Provider credential overlap where supported, restart API, prove new connection, then revoke old | real loopback Redis 7.4.11 atomic contention/expiry/outage/recovery plus safe startup/request failure tests | Managed Redis credential cutover, failover and alert-delivery drill |
...
| CR-13 | `YOUTUBE_API_KEY`, `YOUTUBE_DATA_API_KEY` | Google | backend-only `SecretStr` config → one fixed/bounded YouTube provider client → authenticated channel/organization enrichment and portfolio metadata | Add/restrict the replacement backend key, deploy the backend consumer, move traffic/config from the old alias or key, then revoke old | missing-key fallback, exact-destination, redirect/body/timeout/redaction, OAuth-classification and authenticated-route tests | Google console API/service restriction, quota and live lookup/outage exercise |
| CR-14 | `GOOGLE_PLACES_API_KEY` | Google | Backend `SecretStr` config → fixed redirect-free bounded Places client → authenticated quota-controlled `/me/location/*` → credential-free Next proxies | Create/restrict replacement, configure backend, verify health/live lookup and attribution, then revoke old; frontend does not hold or rotate this key | missing-key fallback, secret-redaction, fixed-destination/redirect/body/timeout, auth/quota and proxy ownership tests | Google console API/service restriction, billing quota, policy review and live lookup/attribution/outage drill |
