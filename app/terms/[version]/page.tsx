...
/**
 * A superseded version of the terms, exactly as it was published.
 *
...
export async function generateMetadata({
  ...
  return {
    title: `Terms ($\{archived.version}) | CreatorJobs`,
    description: `The terms published as version $\{archived.version}.`,
...
export default async function ArchivedTermsPage({
  ...
  const sections = archived.terms as ReadonlyArray<{
    title: string;
    icon: IconName;
    body: string;
  }>;
  ...
  return (
    <main className="min-h-[calc(100vh-56px)] bg-[#0b0b0f] px-4 py-8 text-white sm:px-6">
      <section className="mx-auto max-w-4xl space-y-6">
        <PageHeader
          title={`Terms — version $\{archived.version}`}
          description={`The terms exactly as published in version $\{archived.version}.`}
        />
        ...
        <p className="text-sm text-muted">
          Read the current terms at{" "}
          <Link href="/terms" className="font-semibold text-white/78 hover:text-white hover:underline">
            /terms
          </Link>
          .
        </p>
